...
void Engine::Sprite::Update(float deltaTime)
{
	...
	if (frameDuration >= maxFrameDuration) {//jika sudah melewati interval maka akan direset ke 0 lg
		frameDuration = 0;
	...
	}//jika  .. maka akan dilooping ke awal kalau tidak lanjut ke index berikutnya(+ incremen)
}
...
Engine::Sprite* Engine::Sprite::AddAnimation(string name, int startFrameIndex, int endFrameIndex)
{
	...
	this->animationData.push_back(a);//disimpan disini
	return this;
}
